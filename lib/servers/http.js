...
HTTPServer.Middleware = function (environment) {
    ...
    router.get("*", async (req, res, next) => {
        try {
            ...
            if (error instanceof protocolErrors.PermissionDenied) {
                res.status(403).send(error.message);
            } else if (error instanceof protocolErrors.OperationNotAllowed) {
    ...
    router.put("*", async (req, res, next) => {
        try {
            await environment.updateDocument(req.path, req.body);
            res.status(200).send();
        } catch (error) {        
            if (error instanceof protocolErrors.PermissionDenied) {
                res.status(403).send(error.message);
