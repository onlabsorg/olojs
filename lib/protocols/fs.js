...
const fileProtocol = require('./file');
const protocolErrors = require('./.errors');
...
exports.get = async function (path) {    
    if (isDirectory(path)) {
        ...
        return await fileProtocol.get(path);
    }
}
 ...
 */
exports.set = async function (path, source) {
    if (isDirectory(path)) {
        throw new protocolErrors.OperationNotAllowed('SET', "fs:"+pathlib.join('/', path))
    } else {
        await fileProtocol.set(path, source);
    }
}
...
exports.delete = async function (path) {
    if (isDirectory(path)) {
        throw new protocolErrors.OperationNotAllowed('DELETE', "fs:"+pathlib.join('/', path))
    } else {
        await fileProtocol.delete(path);
    }
}
