...
module.exports = function (env) {
    ...
    config.rootPath = ifString(config.rootPath) || `${__dirname}/public`;
    ...
    router.get("/*/vendors~/bin/:modulename", (req, res, next) => {
        const script = fs.readFileSync(`${config.rootPath}/vendors~/bin/${req.params.modulename}`, "utf8");
        res.status(200).send(script);
    });
    ...
    router.use( express.static(config.rootPath) );    
