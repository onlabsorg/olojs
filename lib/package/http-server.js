...
module.exports = function (env) {
    ...
    router.get("/main.js", (req, res, next) => {
        const script = fs.readFileSync(path.resolve(__dirname, "./http-client/main.js"), "utf8");
        res.status(200).send(script);
    });

    router.get("/*/bin/:modulename", (req, res, next) => {
        const script = fs.readFileSync(path.resolve(__dirname, "./http-client/bin/"+req.params.modulename), "utf8");
        res.status(200).send(script);
    });
    ...
    router.get("/*.html", (req, res, next) => {
        if (!htmlDocument || noCache) {
            htmlDocument = fs.readFileSync(path.resolve(__dirname, "./http-client/index.html"), "utf8");
        }
