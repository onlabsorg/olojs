...
class BackendEnvironment extends Environment {
    ...
    constructor (config={}) {
        ...
        if (!isObject(config.globals)) config.globals = {};
        config.globals.require = modulePath => require(`./stdlib/${modulePath}`);
    ...
    createMiddleware (router) {
        ...
        router.get("*", async (req, res, next) => {
            ...
            if (!(await this.allow(req))) return res.status(403).send();
