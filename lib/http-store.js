...
class HTTPStore extends Store {
    ...
    async get (path) {
        ...
        switch (response.status) {
            ...
            case 403:
                throw new this.constructor.PermissionDeniedError('GET', url);
            case 404:
                return "";
            case 405:
                throw new this.constructor.OperationNotAllowedError('GET', url);
            default:
    ...
    async list (path) {
        ...
        switch (response.status) {
            case 200:
                ...
                else throw new this.constructor.OperationNotAllowedError('LIST', url);
            case 403:
                throw new this.constructor.PermissionDeniedError('LIST', url);
            case 404:
                return [];
            case 405:
                throw new this.constructor.OperationNotAllowedError('LIST', url);
            default:
                let message = await response.text();
                throw new Error(message);
        }                            
    ...
    async set (path, source) {
        ...
        switch (response.status) {
            ...
            case 201:
                break;
            case 403:
                throw new this.constructor.PermissionDeniedError('SET', url);
            case 405:
                throw new this.constructor.OperationNotAllowedError('SET', url);
            default:
                let message = await response.text();
                throw new Error(message);
        }                    
    ...
    async delete (path) {
        ...
        switch (response.status) {
            case 200:
                break;
            case 403:
                throw new this.constructor.PermissionDeniedError('DELETE', url);
            case 405:
                throw new this.constructor.OperationNotAllowedError('DELETE', url);
            default:
                let message = await response.text();
                throw new Error(message);
        }        
