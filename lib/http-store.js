...
class HTTPStore extends Store {
     ...
     *  ------------------------------------------------------------------------
     *  Retrieves the list of items contained under the given path, via 
     *  HTTP GET (HTTPS GET). The server implmentation should implement this
     *  functionality when the request accepts the `application/json` MimeType.
     *
     *  ```js
     *  source = await httpStore.read("/path/to/doc")
     *  ```
     *
     *  - On 200 status code, returns the response item as json object
     *  - On 403 status code, throws a `HTTPStore.ReadPermissionDeniedError`
