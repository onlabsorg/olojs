...
class Environment {
     ...
     */
    async load (path, argns={}) {
        const resourcePath = Path.from(path);
        let [pathHandler, resourceSubPath] = this._findPathHandler(resourcePath);
        
        if (isFunction(pathHandler.load)) {
            return await pathHandler.load(resourceSubPath, argns);
        }
        
        const source = await this.readDocument(path);
        const evaluate = this.parseDocument(source);
        const context = await this.createContext(resourcePath, {argns});
    ...
    createContext (docPath, presets={}) {
        ...
        return document.createContext(this.globals)
                .$assign({
                    "import": (subPath, argns) => this.load(docPath.resolve(subPath), argns)
                })
                .$extend(presets)
                .$assign({
...
class PathHandler {
    ...
    constructor (path, handlers) {
        ...
        if (isFunction(handlers)) {
            ...
            if (isFunction(handlers.load)) this.load = handlers.load.bind(handlers);
        }
    }
    ...
    //  load (path, argns) {
    //      If not defined, the environment will instead `read` a source, 
    //      parse it and return the document namespace.
    //  }
