...
const createRouteHandler = (protocolHandler, rootPath) => ({
    get: (path, options) => protocolHandler.get(pathlib.join('/', rootPath, path), options),
    set: (path, source, options) => protocolHandler.set(pathlib.join('/', rootPath, path), source, options),
    delete: (path, options) => protocolHandler.delete(pathlib.join('/', rootPath, path), options)
});
...
function getURILocation (uri) {
    ...
    return pathlib.join('/', URI.serialize(uri));
}
