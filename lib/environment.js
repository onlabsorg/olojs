...
class Environment {
    ...
    constructor (options) {
        ...
        this.globals.import = async function (id) {
            return await environment._importDocument(this, id);
        }
    }
    ...
    // This method defined the import mechanism used by the context import 
    // function.
    async _importDocument (context, id) {
        const targetPath = pathlib.resolve(context.__path__, isDir(context.__path__) ? '.' : '..', id);
        if (context.$cache.imports.has(targetPath)) {
            var targetDoc = context.$cache.imports.get(targetPath);
        } else {
            var targetDoc = await this.readDocument(targetPath);
    ...
    async listEntries (id) {
        const path = sliceBefore(id, '?');
        return await this.store.list(path);
    }
