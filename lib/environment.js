...
class Environment {
    ...
    async loadDocument (path, presets={}) {
        const modulePath = stdlibPath.getSubPath(path);
        if (modulePath) {
            return await this.constructor._loadModule(modulePath);
        }
    ...
    // this is called by loadDocument("/bin/modulePath") and from
    // import("/bin/modulePath")
