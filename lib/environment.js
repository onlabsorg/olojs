...
class Environment {
    ...
    async readDocument (path) {
        const docPath = Path.normalize(path);
        
        if (this.cache && this.cache.has(docPath)) {
            return this.cache.get(docPath);
        }
        
        const doc = await this.store.read(docPath);
        
        if (this.cache && !Path.isDirectory(docPath)) {
    ...
    async writeDocument (path, source) {
        ...
        if (isFunction(this.store.write)) {
            await this.store.write(docPath, source);
        } else {
            throw new Error(`Write operation not defined`);
        }
    
        if (this.cache && this.cache.has(docPath)) {
            this.cache.set(docPath, source);
        }
    }
    ...
    createContext (docPath, ...namespaces) {
        ...
        const context = document.createContext(
                this.constructor.globals, this.globals, {}, ...namespaces);
        
        context.__path__ = String(docPath);

        const environment = this;
        context.import = function (subPath, ...namespaces) {
            let fullPath = Path.resolve(this.__path__, subPath);
