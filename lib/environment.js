...
class Environment {
    ...
    constructor (config={}) {
        ...
        if (isObject(config.paths)) {
            for (let pathHandlerPath in config.paths) {
                let pathHandler = this._createPathHandler(pathHandlerPath, config.paths[pathHandlerPath]);
    ...
    _createPathHandler (path, handler) {
        ...
        if (isObject(handler) && isFunction(handler.write)) {
            ...
            pathHandler.write = () => {throw new Error(`Write operation not defined for paths ${pathHandler.path}*`)}            
        }

        if (isObject(handler) && isFunction(handler.delete)) {
            pathHandler.delete = handler.delete.bind(handler);            
        } else {
            pathHandler.delete = () => {throw new Error(`Delete operation not defined for paths ${pathHandler.path}*`)}            
        }
