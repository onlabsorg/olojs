...
class Router {
    ...
    resolvePath (path) {
        path = pathlib.joint(`/`, path).slice(1);
        let i = path.indexOf('/');
        if (i === -1) return [nullProtocol, path];
        let name = path.slice(0, i);
        let subPath = path.slice(i);
        let handler = this._tree.get(name) || nullProtocol;
        return [handler, subPath]
    }
    
    async get (path) {
        let [handler, subPath] = this.resolvePath(path);
        return await handler.get(subPath);
    }
    
    async set (path, source) {
        let [handler, subPath] = this.resolvePath(path);
        return await handler.set(subPath, source);        
    }
    
    async delete (path) {
        let [handler, subPath] = this.resolvePath(path);
        return await handler.delete(subPath);                
    }
...
class VFS extends Map {
    ...
    async get (path) {
        let [root, subPath] = splitPath(path);
        if (super.has(root)) {
            return await super.get(root).get(subPath);
        } else {
            return await nullProtocol.get(path);
        }
    }
    ...
    async set (path, source) {
        let [root, subPath] = splitPath(path);
        if (super.has(root)) {
            return await super.get(root).set(subPath, source);
        } else {
            return await nullProtocol.set(path, subPath);
        }
    }
    ...
    async delete (path) {
        let [root, subPath] = splitPath(path);
        if (super.has(root)) {
            return await super.get(root).delete(subPath);
        } else {
            return await nullProtocol.get(path);
        }
    }
