...
class Router extends Store {
    ...
    constructor (routes) {
        super();
        this._routes = {};
        for (let path in this.constructor.defaultRoutes) {
            this.mount(path, this.constructor.defaultRoutes[path]);
        }
    ...
    normalizePath (path) {
        ...
        if (uri) {
            ...
            const uriPath = uri[2];
            return super.normalizePath(`/.protocols/${uriScheme}/${uriPath}`);
        } else {
...
// The routes contained in this object will be added by default to every
// Route instance.
