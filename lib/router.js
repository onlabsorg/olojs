...
class Router extends Store {
    ...
    async list (path) {
        ...
        const entries = [];
        for (let [store, subPath] of this.iterMatches(dirPath)) {
            mergeLists(entries, await store.list(subPath));
        }
        for (let [routePath, store] of this.iterRoutes()) {
    ...
    match (path) {
        const matches = this.iterMatches(path);
        return matches.next().value || [null, path];
    }

    *iterMatches (path) {
        const normPath = this.normalizePath(path);
        for (let [routeId, store] of this.iterRoutes()) {
