 ...
 *  
 *  The use of URI-like paths is allowed, but `scheme:/path/to/doc` is just
 *  syntactic shugar for `/.prot/scheme/path/to/doc`.
 */
class Router extends Store {
    ...
    constructor (routes) {
        ...
        this._routes = {};
        for (let path in routes) this.mount(path, routes[path]);
    }
     ...
     */
    async list (path) {
        const dirPath = normalizeDirPath( normalizeURI(path) );
        const entries = [];
    ...
    createContext (path, presets={}) {
        path = normalizeURI(path);
        const store = this._match(path)[0];
        if (store) {
            const context = store.createContext(path, presets);
            context.$store = this;
            return context;
        } else {
            return super.createContext(path, presets);
        }
    }
    ...
    *_iterRoutes () {
    ...
    mount (path, store) {
        path = normalizeURI(path);
        if (isStore(store)) {
            this._routes[ normalizeDirPath(path) ] = store;
        }
    }
    
    unmount (path) {
        path = normalizeURI(path);
        delete this._routes[ normalizeDirPath(path) ];
    }
}
...
const normalizeDirPath = path => pathlib.normalize(`/${path}/`);

function normalizeURI (uri) {
    const uriMatch = uri.match(URI_RE);
    const scheme = uriMatch ? (uriMatch[1] || '') : '';
    const path = uriMatch ? pathlib.join('/', uriMatch[2] || '') : pathlib.normalize(`/${uri}`);
    return scheme ? pathlib.join(`/.prot/${scheme}`, path) : path;
}