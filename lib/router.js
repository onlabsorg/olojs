...
class Router extends Store {
    ...
    constructor (routes, protocols={}) {
        ...
        for (let [key, value] of Object.entries(protocols)) {
            if (isValidProtocolName(key) && isStore(value)) {
                this._protocols[key] = value;
            }
        }
    }
    ...
    *_iterMatches (path) {
        ...
        // If path starts with a URI, it yields the protocol store
        // matching the scheme (if any) and returns
        const protocolMatch = path.match(new RegExp(`^(${protocolNameRe}):\/(.*)$`));
        if (protocolMatch) {
            const protocolName = protocolMatch[1];
            const store = this._protocols[protocolName];
            if (store) {
                const subPath = this.normalizePath(protocolMatch[2]);
                yield [store, subPath];
            }
            return;
        }

        // If path is a simple path, it yields all the routed stores that
        // match the path
...
const protocolNameRe = "[a-zA-Z][a-zA-Z0-9+.-]*";
const isValidProtocolName = name => (new RegExp(`^${protocolNameRe}$`)).test(name);
