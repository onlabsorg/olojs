 ...
 */
exports.ViewerMiddleware = function (storeURL) {
    const router = express.Router();
    ...
    router.get('/info', (req, res, next) => {
        res.status(200).json({
            'storeURL': storeURL,
            'olojsVersion': require('../package.json').version
        })
    });
...
exports.createServer = function (store, options={}) {
    const storePath = options.storePath || '/docs';    
    
    const app = express();    
    app.use(storePath, this.StoreMiddleware(store));
    app.use('/', this.ViewerMiddleware(storePath));
