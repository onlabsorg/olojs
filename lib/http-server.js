 ...
 *  ```
 */
exports.StoreServer = store => createServer( StoreMiddleware(store) );
 ...
 */
const ViewerMiddleware = exports.ViewerMiddleware = function (store) {
    const router = express.Router();    
    router.use('/docs', StoreMiddleware(store));
    router.use('/', express.static( pathlib.join(__dirname, '../browser') ));
    return router;
}
 ...
 *  ```
 */
exports.ViewerServer = store => createServer( ViewerMiddleware(store) );





// Converts a middleware to a HTTP Server
