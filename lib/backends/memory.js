...
MemoryBackend.getUserRights = function (hostName, collectionName, docId) {
    return rights.WRITE;
}
...
MemoryBackend.Document = class extends AbstractBackend.Document {
    ...
    get readable () {
        const getUserRights = this.store.constructor.getUserRights;
        return getUserRights(this.store.host, this.collection, this.id) >= rights.READ;
    }

    get writable () {
        const getUserRights = this.store.constructor.getUserRights;
        return getUserRights(this.store.host, this.collection, this.id) >= rights.WRITE;
    }
