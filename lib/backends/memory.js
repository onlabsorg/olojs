...
MemoryBackend.Document = class extends AbstractBackend.Document {
    ...
    async open () {
        ...
        var  doc = this.store._data[this.id];
        if (!doc) {
            this._assertWritable();
            doc = this.store._data[this.id] = {};
        }
    ...
    get readable () {
        return this.store.getUserRights(this.id) >= rights.READ;
    }

    get writable () {
        return this.store.getUserRights(this.id) >= rights.WRITE;
    }
