...
OlodbBackend.Document = class extends AbstractBackend.Document {
    ...
    constructor (store, docId) {
        super(store, docId);
        const parsedId = docId.split(".");
        this.collection = parsedId[0];
        this.name = parsedId[1];
    }

    async open () {
        this._shareDoc = this.store._sharedb.get(this.collection, this.name);
        this._permissions = await this.store._call('getUserRights', this.collection, this.name);
