...
class Document {
    ...
    canWrite (...path) {
        if (this.writable === true) return true;
        if (this.writable === false) return false;
        var permissionLevel = Number(this.writable);
        var protectionLevel = 0;
        path = new Path(...path);
        for (let key of path) {
            protectionLevel = Math.max(protectionLevel, protectionLevelOf(key));
        }
        if (this.type(path) === "dict") {
            protectionLevel = Math.max(protectionLevel, deepProtectionLevelOf(this, path));
        }
        return protectionLevel <= permissionLevel;
    }
...
function protectionLevelOf (key) {
    var match = key.match(/^(_*).*/);
    return match ? match[1].length : 0;
}


function deepProtectionLevelOf (doc, path) {
    var protectionLevel = 0;
    var keys = doc.getDictKeys(path);
    for (let key of keys) {
        protectionLevel = Math.max(protectionLevel, protectionLevelOf(key));
        if (doc.type([path, key]) === "dict") {
            protectionLevel = Math.max(protectionLevel, deepProtectionLevelOf(doc, new Path(path, key)));
        }
    }
