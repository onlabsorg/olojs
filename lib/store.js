...
class Store {
     ...
     */
    createContext (docId) {
        const {path, argns} = this.constructor.parseId(docId);
        return Context(this, path, argns);
    }
...
const Context = (store, path, argns) => document.createContext(
    
    contextPrototype,
    
    {    
        __read__    : store.read.bind(store),        
        __list__    : store.list.bind(store),
        __context__ : store.createContext.bind(store),
        $cache      : new Map(),
    },
...
const contextPrototype = {
    ...
    __parse__: document.parse.bind(document),
    
    async import (docId) {
        const targetId = resolveId(this.__path__, docId);
        const targetContext = this.__context__(targetId);
        const targetPath = targetContext.__path__;
        if (!this.$cache.has(targetPath)) {
            const source = await this.__read__(targetPath);
            const evaluate = this.__parse__(source);
