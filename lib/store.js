...
class Store {
     ...
     *
     *  - `doc.path`: the normalized path of the document
     ...
     *    current store, evaluates it and returns its namespace.
     */
    create (docPath, source="") {
        return new this.constructor.Document(this, docPath, source);
    }
     ...
     *  ------------------------------------------------------------------------
     *  Given a document path, reads a document source and creates the
...
Store.Document = class {
    ...
    get path () {
        return this.store.normalizePath(this._path);
    }
    ...
    async import (path) {
        const targetPath = this.store.resolvePath(this.path, path);
        if (!this.cache.has(targetPath)) {
            const source = await this.store.read(targetPath);
            const evaluate = this.store.create(targetPath, source).parse();
            this.cache.set(targetPath, {source, evaluate});
        }
        const {source, evaluate} = this.cache.get(targetPath);
        const context = this.store.create(targetPath).createContext();
