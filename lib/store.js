    ...
    createContext (path, presets={}) {
        const context = document.createContext({
            ...
            async import (docId) {
                ...
                const targetPath = context.$store.resolvePath(context.__path__, path);
                if (!context.$cache.has(targetPath)) {
                    const source = await context.$store.read(targetPath);
                    const evaluate = document.parse(source);
                    context.$cache.set(targetPath, {source, evaluate});
                }
                const {source, evaluate} = context.$cache.get(targetPath);
                const target_context = context.$store.createContext(targetPath, {__query__:query});
                const {text, data} = await evaluate(target_context);
                data.__str__ = () => text;
                return data;
            }
        });
