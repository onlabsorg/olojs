...
const parse = Parser({
     ...
     binaryOperations: {
         ...
         "if"   :   {precedence:22, handler:"$if"  },
         "or"   :   {precedence:23, handler:"$or"  },
         "and"  :   {precedence:23, handler:"$and" },
...
const context = {
    ...
    async $if (X, Y) {
        const y = await Y(this);
        return (await this.bool(y)) ? await X(this) : NOTHING;
    },
