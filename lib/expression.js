...
const $NS_CONTEXT = Symbol("Child namespace context");
...
const context = {
    ...
    async $dot (X, Y) {
        ...
        return namespaces.map(async namespace => {
            ...
            if (!namespace[$NS_CONTEXT]) {
                namespace[$NS_CONTEXT] = Object.assign(Object.create(context), namespace);
            }
            return await Y(namespace[$NS_CONTEXT]);
        });
    },
