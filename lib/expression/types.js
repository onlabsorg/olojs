...
function createRange (x, y) {
    const a = normalize(x), aType = detectType(a);
    const b = normalize(y), bType = detectType(b);
    if (aType !== 'NUMBER' || bType !== 'NUMBER') {
        throw new Exception(`Range operation not defined between ${aType} and ${bType}`);
    }
    return new Tuple(...generateRange(Math.trunc(a), Math.trunc(b)))
}
...
module.exports = {
    Tuple, NOTHING, normalize, createRange,
