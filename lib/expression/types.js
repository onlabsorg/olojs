...
function createTupleFromPairs (tuple1, tuple2) {
    return createTuple(...pair(tuple1, tuple2));
}

const NOTHING = createTuple();

function normalizeTuple (tuple) {
    if (!isTuple(tuple)) return tuple;
...
function createRange (x, y) {
    const a = normalizeTuple(x), aType = detectType(a);
    const b = normalizeTuple(y), bType = detectType(b);
    if (aType !== 'NUMBER' || bType !== 'NUMBER') {
        throw new Exception(`Range operation not defined between ${aType} and ${bType}`);
    }
    return createTuple(...generateRange(Math.trunc(a), Math.trunc(b)))
}

function *generateRange (a, b) {
    if (a === b) yield a;
    else if (a <= b) for (let i=a; i<=b; i++) yield i;
...
async function map (fn, tuple) {
    ...
    for (let item of tuple) {
        retVals = createTuple(retVals, await fn(item));
    }
    return retVals;
}

function mapPairs (fn, x, y) {
    const ptuple = createTupleFromPairs(x, y);
...
function compare (x, y) {
    ...
    if (xType === 'TUPLE') return lexCompare(x, createTuple(y));
    if (yType === 'TUPLE') return lexCompare(createTuple(x), y);
    switch (`${xType}-${yType}`) {
...
function isEqual (x, y) {
    ...
    if (xType === 'TUPLE') return isLexEqual(x, createTuple(y));
    if (yType === 'TUPLE') return isLexEqual(createTuple(x), y);
    switch (`${xType}-${yType}`) {
        case 'BOOLEAN-BOOLEAN':     return x === y;
        case 'NUMBER-NUMBER':       return x === y;
        case 'STRING-STRING':       return x === y;
        case 'FUNCTION-FUNCTION':   return x === y;
        case 'LIST-LIST':           return isLexEqual(createTuple(...x), createTuple(...y));
        case 'NAMESPACE-NAMESPACE': {
...
function apply (x, y) {
    const xType = detectType(x);
    switch (xType) {
        case 'FUNCTION':  return x.call(this, ...createTuple(y));
        case 'STRING':    return getListItem(x, y) || "";
        case 'LIST':      return getListItem(x, y) || NOTHING;
        case 'NAMESPACE': {
        ...
        case 'TUPLE':     return map(xi => apply.call(this, xi, y), x);
...
function detectType (value) {
    ...
    // if iterable
    if (typeof value[Symbol.iterator] === "function") return "TUPLE";
...
function isNothing (value) {
    value = normalizeTuple(value);
    return detectType(value) === 'NOTHING';
}

function isTuple (value) {
    return detectType(value) === 'TUPLE';
}
...
function convertToTuple (x) {
    switch (detectType(x)) {
        case 'STRING':      return createTuple(...Array.from(x))
        case 'LIST':        return createTuple(...x);
        case 'NAMESPACE':   return createTuple(...Object.getOwnPropertyNames(x));
        default:            return x;
    }    
}
...
module.exports = {
    NOTHING, createTuple, createTupleFromPairs, normalizeTuple, createRange, map, mapPairs,
    add, sub, mul, div, mod, pow,
    compare, isEqual,
    apply,
    detectType, isValidName, isNothing, isTuple,
...
function getListItem (list, index) {
    index = normalizeTuple(index);
    if (detectType(index) !== 'NUMBER') return null;
...
function *pair (x, y) {
    const iX = createTuple(x)[Symbol.iterator]();
    const iY = createTuple(y)[Symbol.iterator]();
    while (true) {
        let x = iX.next();
        let y = iY.next();
