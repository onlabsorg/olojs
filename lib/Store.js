...
class Store {
    ...
    connect (credentials) {
        ...
        return co(function* () {
            if (!self.connected) {
                self.userId = yield self.__connect(credentials);
            }
        });
    }
     ...
     *    
     *  ###### Resolves
     *  - the logged-in user id if the connection was successfull
     *    
     *  ###### Rejects
    ...
    /**
     *  ### User.prototype.getUserRole(docId) - async method
     *  The `.getUserRole` method retrieves the access control role of the 
     *  logged-in user on a specific document.
     *
     *  ###### Parameters
     *  - `docId` the id of the document for which we want to know the user role
     *    
     *  ###### Resolves
     ...
     *  - `roles.NONE` if the user has no permissions on the document
     *    
     *  > * The root-level keys of a document are meta-data, while the root key `data`
     *  > contains the document data.
     *  > * By default this method resolves always `roles.OWNER` and it shoul
     *  > be overridden by the implementations to define a proper access control.
     */
    getUserRole (docId) {
        return Promise.resolve(roles.OWNER);
    }
...
class Document {
     ...
     *  This method is called by the `Store.prototype.getDocument` method 
     *  and should contain the asynchronous initialization
    ...
    open () {
        ...
        return co(function* () {
            ...
            self.state = "opening";
            self._userRole = yield self.store.getUserRole(self.id);
            try {
                if (self.userRole < roles.READER) throw new errors.ReadPermissionError(self.id);
                const content = yield self.__fetch();
                if (content === null) {
                    if (self.userRole < roles.OWNER) throw new errors.WritePermissionError(self.id);
                    yield self.__create({});
                }
                self.state = "open";
            }
     ...
     */
    __fetch () { return Promise.resolve(null) }
     ...
     *  - `Error` if the operation failed
     */
    __create (content) { return Promise.resolve() }


    /**
     *  ### Document.prototype.userRole - getter
     *  This getter holds the result of `Store.prototype.getUserRole`
     *  after the document is open.
     *  
     *  ###### Returns
     *  - `roles.OWNER` if the user has write permissions on this document data and metadata
     *  - `roles.WRITER` if the user has write permissions on this document data
     *  - `roles.READER` if the user has read-only permissions on this document
     *  - `roles.OWNER` if the user has no permissions on this document
     */
    get userRole () {
    ...
    //
    _assertReadable () {
        if (this.userRole >= roles.READER) return this;
    ...
    _assertWritable (path) {
        if (this.userRole >= roles.OWNER) return true;
        if (Path.from(path)[0] === "data" && this.userRole >= roles.WRITER) return true;
...
class Item {
    ...
    /**
     *  ### Item.prototype.readonly - getter
     *
     *  ###### Returns
     *  - `true` if the user has no write permissions on this item
     *  - `false` otherwise
     */
    get readonly () {
        try {
            this.doc._assertWritable(this.path);
            return false;
        }
        catch (error) {
            return true;
...
class Dict extends Item {
     ...
     */
    set (key, value) {
        const path = new Path(this.path, key)
        this.doc._assertOpen()._assertWritable(path);

        key = String(key);
        value = Validator.value(value);

        const oldValue = this.doc._getItemValue(path);
...
class List extends Item {
     ...
     */
    set (index, item) {
        const path = new Path(this.path, index)
        this.doc._assertOpen()._assertWritable(path);

        index = Validator.index(index, this.size);
        item = Validator.value(item);

        const oldItem = this.doc._getItemValue(path);
...
class Text extends Item {
     ...
     */
    insert (index, string) {
        this.doc._assertOpen()._assertWritable(this.path);
        index = Validator.index(index, this.size, 1);
