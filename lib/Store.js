...
class Store {
    ...
    connect (credentials) {
        ...
        return co(function* () {
            if (!self.connected) {
                self._user = yield self.__connect(credentials);
            }
            return self.user;
        });
    }
...
class Document {
    ...
    open () {
        ...
        return co(function* () {
            ...
            try {
                ...
                if (content === null) yield self.__create({});
                self.state = "open";
            }
            catch (err) {
                self.state = "closed";
    ...
    _assertReadable () {
        ...
        if (role >= roles.READER) return this;
    ...
    _assertWritable (path) {
        ...
        if (Path.from(path)[0] === "data" && role >= roles.WRITER) return true;
