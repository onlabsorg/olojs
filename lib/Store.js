...
class Document {
    ...
    isWritable (path) {
        const role = this.__getUserRole();
        return this._isMeta(path) ? role >= roles.OWNER : role >= roles.WRITER;
    }
    ...
    //
    //  returns true if the path points to meta-data
    //
    _isMeta (path) {
        path = Path.from(path);
        return path[0] !== "root";
    ...
    _getItem (rootPath, subPath) {
        ...
        this._assertOpen()._assertReadable(itemPath);
    ...
    _getItemValue (...path) {
        path = new Path(...path);
        this._assertOpen()._assertReadable(path);
...
class Dict extends Item {
    ...
    set (key, value) {
        ...
        this.doc._assertOpen()._assertWritable([this.path, key]);
...
class List extends Item {
    ...
    set (index, item) {
        ...
        this.doc._assertOpen()._assertWritable([this.path, index]);
