...
class Document {
    ...
    constructor (store, path, source="") {
        ...
        this.$evaluate_cache = null;
    }

    get evaluate () {
        if (!this.$evaluate_cache) {
            this.$evaluate_cache = parse(this.source);
        }
        return this.$evaluate_cache;
    }
    ...
    createContext (...presets) {
        ...
        context.import = async (targetPath, presets={}) => {
            ...
            if (!targetDoc) {
                targetDoc = await load(this.store, fullTargetPath);
...
async function load (store, path) {
    ...
    return new Document(store, path, source);
}


module.exports = {parse, createContext, Document, load}
