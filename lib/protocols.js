...
class Protocols extends Router {
    ...
    static parseId (docId) {
        const uri = parseURI(docId);
        const id = super.parseId(uri.path);
        if (uri.scheme === 'default') return id;
        else return {
            path: `${uri.scheme}:${id.path}`,
            argns: id.argns
        }
    }
}
...
function parseURI (uri) {
    ...
    if (!uriMatch) return {
        scheme: 'default', 
        path: pathlib.normalize(`/${uri}`)
    }; 
    else return {
        scheme: uriMatch[1] || 'default',
        path: pathlib.join('/', uriMatch[2] || '')
    }
}
